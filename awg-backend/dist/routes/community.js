...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/posts/:id/comments", auth_1.optionalAuth, async (req, res) => {
    try {
        const postId = parseInt(req.params.id, 10);
        if (isNaN(postId)) {
...
router.get("/admin/posts", auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
...
router.delete("/admin/posts/:id", auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const postId = parseInt(req.params.id);
        const postRepo = data_source_1.AppDataSource.getRepository(CommunityPost_1.CommunityPost);
        const userRepo = data_source_1.AppDataSource.getRepository(User_1.User);
...
router.patch("/admin/posts/:id/toggle-approve", auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const postId = parseInt(req.params.id);
        const postRepo = data_source_1.AppDataSource.getRepository(CommunityPost_1.CommunityPost);
        const post = await postRepo.findOne({ where: { id: postId } });
...
router.get("/admin/reports", auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const reportRepo = data_source_1.AppDataSource.getRepository(PostReport_1.PostReport);
...
router.delete("/admin/reports/:id", auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
